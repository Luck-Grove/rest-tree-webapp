...
const TreeNode = ({
  ...
  // Skip nodes that have no active layers if the flag is set
  ...
  // Function to return the appropriate icon based on the node type
  const getIcon = (type) => {
    switch (type) {
      case 'folder':
        return (
  ...
  // Checkbox change handler for selecting layers
    ...
    <div
      ...
      onContextMenu={(e) => {
        if (!e.target.closest('.download-button')) {
          handleContextMenu(e, nodeId, setContextMenu);
        }
      }}
      >
      <div className="flex items-center justify-between p-1 rounded-md tree-node fade-in">
        <div className="flex items-center">
          {/* Show checkboxes if it's a layer or if it has layer descendants */}
          {(isLayer || childNodes.length > 0) && (
            <input
              type="checkbox"
              checked={isLayer ? selectedLayers.has(nodeId) : childNodes.some((child) => child && selectedLayers.has(child.props.nodeId))}
